...
If the file master.yaml which contains the public/private key pair generated by the controller is compromised, then all the SealedSecret manifests can be unsealed and the encrypted sensitive information they store revealed. Hence, this file must be guarded by granting least privilege access.
...
Deploy a Jenkins with helm in lib/jenkins.ts to tests. See secrets for credentials:

```s
$ kubectl get secret --namespace default awesomecdkstackjenkinsekswebserver253648b5 -o jsonpath="{.data.jenkins-admin-password}" | base64 --decode

$ kubectl get secret --namespace default awesomecdkstackjenkinsekswebserver253648b5 -o jsonpath="{.data.jenkins-admin-user}" | base64 --decode
```